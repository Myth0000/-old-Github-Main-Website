...
export default class Slideshow extends React.Component {
  constructor(props) {
    ...
    this.state = {
      slideShowIndex:
        props.SlideshowIndex != undefined ? props.SlideshowIndex : 0,
  ...
  componentDidUpdate() {
    if (this.props.Images != this.state.images) {
      this.setState({
        slideShowIndex:
          this.props.SlideshowIndex != undefined
            ? this.props.SlideshowIndex
            : 0,
        images: this.props.Images,
      });
    }
  ...
  getImageIndicators() {
    return this.props.Images.map((image, index) => {
      if (this.state.slideShowIndex === index) {
        return <ImageIndicator key={index} onImage="true" />;
      } else {
        return <ImageIndicator key={index} />;
      }
    });
  }
    ...
    return (
      <div className="slideshow">
        <img
          ...
          src={this.props.Images[this.state.slideShowIndex]}
        />

        <div className="imageNavigationButtons">
          <img
            className="imageNavigationButton"
            src={forwardArrow}
            alt="forward arrow"
            draggable="false"
...
function ImageIndicator(props) {
  let slideshowImageIndicatorClassName =
    document.documentElement.clientWidth > 500
      ? "slideshowImageIndicator"
      : "slideshowImageIndicatorMobile";
  let slideshowImageIndicatorOnImageClassName =
    document.documentElement.clientWidth > 500
      ? "slideshowImageIndicatorOnImage"
      : "slideshowImageIndicatorOnImageMobile";

  if (props.onImage) {
    return <div className={slideshowImageIndicatorOnImageClassName}></div>;
  } else {
    return <div className={slideshowImageIndicatorClassName}></div>;
  }
}
